package DB::BarcoLNSPBSchema::Result::Afisha;

use Modern::Perl;
use base qw/DBIx::Class::Core/;

# Associated table in database
__PACKAGE__->table('afisha');
...
for(my $i=1;$i<=3;$i++) {
	$columns{"subheader$i"} = { data_type => 'text', is_nullable => 1 };
	$columns{"foto$i"} = { data_type => 'text', is_nullable => 1 };
	$columns{"place$i"} = { data_type => 'text', is_nullable => 1 }
		if $i < 3;
}
